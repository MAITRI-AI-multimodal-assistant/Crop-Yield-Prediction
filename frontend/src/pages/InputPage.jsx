...
import { Leaf, Mic, ScanLine, ChevronDown } from "lucide-react";

const FLASK_URL = "http://192.168.29.252:8000";
...
const STATES  = [
  ...
  "Uttar Pradesh", "Maharashtra", "Karnataka", "Andhra Pradesh",
  "Tamil Nadu", "Madhya Pradesh", "Rajasthan", "Punjab", "Haryana", "Gujarat",
];

const inputClass =
  "w-full px-4 py-2.5 rounded-xl bg-black/40 border border-white/10 text-white placeholder-gray-600 text-sm focus:outline-none focus:border-green-500/50 focus:bg-green-500/5 focus:shadow-[0_0_0_3px_rgba(0,166,81,0.12)] transition-all";
const labelClass =
  "block text-[0.72rem] font-bold tracking-[0.12em] uppercase text-gray-400 mb-1.5";
    ...
    <div>
      ...
      <div className="relative">
        ...
        >
          ...
          {options.map((o) => (
            <option key={o} value={o} style={{ background: "#0d1810" }}>{o}</option>
          ))}
        </select>
...
export default function InputPage() {
  ...
  useEffect(() => {
    if (formData.season && formData.annual_rainfall) {
      ...
      const base =
        formData.season === "Kharif" ? 0.7
        : formData.season === "Rabi" ? 0.55
        : 0.6;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // ← FIXED: payload was missing, now properly defined from formData
      const payload = {
        Crop:            formData.crop,
        ...
        rainfall_nasa:   Number(formData.annual_rainfall),
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-10">
        ...
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-green-500/10 border border-green-500/20 mb-4">
            ...
            <span className="text-[0.62rem] font-bold tracking-[0.16em] uppercase text-green-400">
              ML Yield Prediction
            </span>
          </div>
          ...
          <p className="text-gray-400 text-sm max-w-md mx-auto">
            Fill in your crop, climate, and soil details to get an ML-powered yield prediction.
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-3 justify-center mb-8">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-xl border text-sm font-bold tracking-wide uppercase transition-all duration-200 ${
              showVoice
                ? "bg-green-500/15 border-green-500/40 text-green-400"
                : "bg-white/3 border-white/10 text-gray-400 hover:border-green-500/30 hover:text-green-400"
            }`}
          >
        ...
        {showVoice && (
          <div className="mb-6">
            <VoiceInput lang={voiceLang} onFill={handleVoiceFill} onClose={() => setShowVoice(false)} />
          </div>
        )}
        {showScan && (
          <div className="mb-6">
            <StripScanner onFill={handleScanFill} onClose={() => setShowScan(false)} />
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {/* Climate */}
          <SectionCard icon={Leaf} title="Climate Data" color="from-teal-500 to-green-600">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <FormInput label="Annual Rainfall (mm)" name="annual_rainfall" type="number" value={formData.annual_rainfall} onChange={handle} placeholder="e.g. 1400" required />
              <FormInput label="Temperature (°C)"     name="temperature"     type="number" value={formData.temperature}     onChange={handle} placeholder="e.g. 28"   required />
              <FormInput label="Humidity (%)"         name="humidity"        type="number" value={formData.humidity}        onChange={handle} placeholder="e.g. 75"   required />
            </div>
          </SectionCard>
          ...
          <SectionCard icon={Leaf} title="Farm Inputs" color="from-green-400 to-teal-500">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <FormInput label="NDVI (auto-calculated)"  name="ndvi"       type="number" value={formData.ndvi}       onChange={handle} placeholder="0.2–0.95" />
            </div>
          </SectionCard>
